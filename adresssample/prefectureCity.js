function getPrefectureSelection(prefSelectionId, citySelectionId, jsonPath, valType='code', initPrefVal=null, initCityVal=null) {
    // JSONファイルから都道府県を取得
    $.getJSON(jsonPath, function (data) { //JSONからjsonpathのデータを持ってくる
      $.grep(data, //dataの絞り込み
        function (obj, idx) {
          // Get the prefecture.
          if (!obj.cityName) {//Citynameが無い場合を除いている
            let selected = '';
            if (initPrefVal && (
    ...
    .fail(function(jqXHR, textStatus, errorThrown) {
      // エラーハンドリング
      console.error("JSONデータの取得に失敗しました:", errorThrown);
    });
  }
  ...
  function getCitySelection(prefSelectionId, citySelectionId, jsonPath, valType='code', initPrefVal=null, initCityVal=null, selectedPrefCode=null) {
    // 県を選んだ時市区町村リストのリセット最初のoptionタグ以外リセット
